  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-semibold text-[var(--text)]">Rituals</h2>
        <button
          type="button"
      ...
      {rituals.map((r) => (
        <div key={r.id} className="rounded-[var(--radius-xl)] border border-[var(--border)] bg-[var(--surface)] p-4 shadow-[var(--shadow-sm)]">
          {editingId === r.id ? (
            <RitualForm
              ritual={r}
              onSave={(updates) => {
            ...
            <div className="flex items-start justify-between">
              <div>
                <h3 className="font-medium text-[var(--text)]">{r.name || 'Untitled'}</h3>
...
function RitualForm({ ritual, onSave, onCancel }) {
  ...
  const [scheduleType, setScheduleType] = useState(ritual.schedule?.type ?? 'daily')
  const [questions, setQuestions] = useState(ritual.questions ?? ['', '', ''])
  const [suggestedActions, setSuggestedActions] = useState(ritual.suggestedActions ?? ['', '', ''])
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        <label className="block text-xs font-medium text-[var(--text-secondary)]">3 questions</label>
        {[0, 1, 2].map((i) => (
          <input
            key={i}
            type="text"
      ...
      <div>
        <label className="block text-xs font-medium text-[var(--text-secondary)]">Suggested actions</label>
        {[0, 1, 2].map((i) => (
          <input
            key={i}
            type="text"
      ...
      <div className="flex gap-2">
        <button type="submit" className="rounded-[var(--radius-md)] bg-[var(--accent)] px-4 py-2 text-sm font-medium text-white transition-[var(--transition)] hover:bg-[var(--accent-hover)]">
          Save
        </button>
        <button type="button" onClick={onCancel} className="rounded-[var(--radius-md)] px-4 py-2 text-sm text-[var(--muted)] transition-[var(--transition)] hover:text-[var(--text)]">
          Cancel
        </button>
      </div>
    </form>
