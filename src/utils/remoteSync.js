...
export async function fetchRemoteState() {
  ...
  if (!url) {
    if (LOG_SYNC)
      console.warn("[TARS sync] No API URL (same origin or set VITE_API_URL)");
    return null;
  }
  ...
  try {
    ...
    if (LOG_SYNC)
      console.log(
        "[TARS sync] Loaded remote state:",
        Object.keys(raw)
          .map((k) => `${k}:${raw[k]?.length ?? 0}`)
          .join(", "),
      );
    return {
      ...
      meetingSheets:
        raw.meetingSheets && typeof raw.meetingSheets === "object"
          ? raw.meetingSheets
          : {},
    };
  } catch (e) {
...
export async function pushRemoteState(state) {
  ...
  if (LOG_SYNC)
    console.log(
      "[TARS sync] Pushing to:",
      url,
      Object.keys(payload)
        .map((k) => `${k}:${payload[k]?.length ?? 0}`)
        .join(", "),
    );
  try {
